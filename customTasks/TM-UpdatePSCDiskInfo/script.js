...
Description:
	...
	//INPUTS:
		...
		CustomerName: [Generic Text Input] Mandatory
		ouname:[Generic Text Input] Mandatory
...
function getAccount(accountName){
  ...
  /**
     ...
     * @return None
     */
function addVMDisk(PSCIP,PSCPort,PSCProtocol,PSCUser,PSCPassword,SITableName,ouname,CustomerName,VMName,SCSIID,DiskLabel,DiskSize)
{
	//Get the VM Information
	var Status = "Active";
	var fullName = CustomerName + ":" + VMName + ":" + SCSIID
	var FriendlyName = DiskLabel + "-" + SCSIID
	if (PSCProtocol == "http"){
...
var PSCVMName = addVMDisk(PSCIP,PSCPort,PSCProtocol,PSCUser,PSCPassword,SITableName,ouname,CustomerName,VMName,SCSIID,DiskLabel,DiskSize);
//Load the Rollback Task.
registerUndoTask(PSCCredentials,PSCIP,SITableName,CustomerName + ":" + VMName + ":" + SCSIID);
